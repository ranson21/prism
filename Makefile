...
# Re-run feature engineering on AWS ECS.
# Use after changing feature formulas in compute.py, or to refresh the feature window.
# Follows with aws-retrain to propagate updated features through scoring.
aws-features:
	$(eval ENV    ?= dev)
	$(eval REGION ?= us-east-1)
	$(eval CLUSTER = prism-$(ENV))
	...
	@echo "→ Running feature engineering in single Fargate task..."
	@TASK_ARN=$$(aws ecs run-task \
	  --cluster $(CLUSTER) --task-definition $(TASKDEF) --launch-type FARGATE \
	  --network-configuration "{\"awsvpcConfiguration\":{\"subnets\":[\"$(SUBNET)\"],\"securityGroups\":[\"$(SG)\"],\"assignPublicIp\":\"ENABLED\"}}" \
	  --overrides '{"containerOverrides":[{"name":"ml-engine","command":["python","-m","app.features.compute"]}]}' \
	...
	[ "$$EXIT" = "0" ] && echo "✓ Features complete" || (echo "✗ Task failed (exit $$EXIT)" && exit 1)

# Retrain the model and rescore all counties without re-ingesting data.
# Use after changing COMPOSITE_WEIGHTS or FEATURE_COLUMNS in train.py.
# Runs train+score in a single Fargate task so the artifact persists in memory.
aws-retrain:
	$(eval ENV    ?= dev)
	$(eval REGION ?= us-east-1)
	$(eval CLUSTER = prism-$(ENV))
	...
	$(eval TASKDEF = $(shell aws ecs describe-services --cluster $(CLUSTER) --services ml-engine \
	  --region $(REGION) --query "services[0].taskDefinition" --output text))
	@echo "→ Running retrain+score in single Fargate task..."
	@TASK_ARN=$$(aws ecs run-task \
	  --cluster $(CLUSTER) --task-definition $(TASKDEF) --launch-type FARGATE \
	  --network-configuration "{\"awsvpcConfiguration\":{\"subnets\":[\"$(SUBNET)\"],\"securityGroups\":[\"$(SG)\"],\"assignPublicIp\":\"ENABLED\"}}" \
	  --overrides '{"containerOverrides":[{"name":"ml-engine","command":["python","-m","app.retrain_score"]}]}' \
	  --region $(REGION) --query "tasks[0].taskArn" --output text); \
	TASK_ID=$$(basename $$TASK_ARN); \
	echo "  Task: $$TASK_ID"; \
	while true; do \
	...
	EXIT=$$(aws ecs describe-tasks --cluster $(CLUSTER) --tasks $$TASK_ID \
	  --region $(REGION) --query "tasks[0].containers[0].exitCode" --output text); \
	[ "$$EXIT" = "0" ] && echo "✓ Retrain complete" || (echo "✗ Task failed (exit $$EXIT)" && exit 1)

# Run the full ML pipeline (ingest → features → train → score) as a one-off Fargate task.
# Use after deploying a new ml-engine image or re-seeding counties.
aws-pipeline:
	$(eval ENV    ?= dev)
	$(eval REGION ?= us-east-1)
	$(eval CLUSTER = prism-$(ENV))
	...
	$(eval TASKDEF = $(shell aws ecs describe-services --cluster $(CLUSTER) --services ml-engine \
	  --region $(REGION) --query "services[0].taskDefinition" --output text))
	@echo "→ Running full pipeline (ingest→features→train→score) in Fargate task..."
	@TASK_ARN=$$(aws ecs run-task \
	  --cluster $(CLUSTER) --task-definition $(TASKDEF) --launch-type FARGATE \
	  --network-configuration "{\"awsvpcConfiguration\":{\"subnets\":[\"$(SUBNET)\"],\"securityGroups\":[\"$(SG)\"],\"assignPublicIp\":\"ENABLED\"}}" \
	  --overrides '{"containerOverrides":[{"name":"ml-engine","command":["python","-m","app.pipeline"]}]}' \
	  --region $(REGION) --query "tasks[0].taskArn" --output text); \
	TASK_ID=$$(basename $$TASK_ARN); \
	echo "  Task: $$TASK_ID"; \
	while true; do \
