...
dependency "rds" {
  config_path = "../rds"
  mock_outputs = {
    db_instance_address                = "prism-dev.abc123.us-east-1.rds.amazonaws.com"
...
dependency "s3" {
  config_path = "../s3"
  mock_outputs = {
    s3_bucket_id = "prism-dev-ml-artifacts"
  }
  mock_outputs_allowed_terraform_commands = ["validate", "plan"]
}
    ...
    ml-engine = {
      ...
      container_definitions = {
        ml-engine = {
          ...
          # ml-engine writes model artifacts to /app/artifacts — needs writable FS
